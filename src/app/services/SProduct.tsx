  ...
  return products;
    ...
    export const getProductsByCategoryParent = async (parentCategoryId: string): Promise<IProduct[]> => {
      ...
      try {
        ...
        const resParentCategory = await fetch(`https://fiyo-be.onrender.com/api/products/category/${parentCategoryId}`);
        const parentCategoryProducts = await resParentCategory.json();

        // Lấy danh sách danh mục con từ danh mục cha
        const resChildCategories = await fetch(`https://fiyo-be.onrender.com/api/category/children/${parentCategoryId}`);
        ...
        const childProductsPromises = childCategories.map(async (category: any) => {
          const resChildProducts = await fetch(`https://fiyo-be.onrender.com/api/products/category/${category._id}`);
...
export const getGroupedSaleProducts = async (
  ...
  try {
    // 1. Lấy sản phẩm từ danh mục cha
    const resParentProducts = await fetch(`https://fiyo-be.onrender.com/api/products/category/${parentCategoryId}`);
    ...
    const childrenData = await Promise.all(
      selectedChildren.map(async (cate) => {
        const res = await fetch(`https://fiyo-be.onrender.com/api/products/category/${cate._id}`);
    ...
    export const getRelatedProducts = async (productId: string): Promise<IProduct[]> => {
      try {
        const res = await fetch(`https://fiyo-be.onrender.com/api/products/related/${productId}`);
        if (!res.ok) {
