  ...
  export const getAllSaleProduct = async (url: string): Promise<IProduct[]> => {
    ...
    export const getProductsByCategoryParent = async (parentCategoryId: string): Promise<IProduct[]> => {
      ...
      try {
        ...
        const resParentCategory = await fetch(`http://localhost:3000/products/category/${parentCategoryId}`);
        const parentCategoryProducts = await resParentCategory.json();

        // Lấy danh sách danh mục con từ danh mục cha
        const resChildCategories = await fetch(`http://localhost:3000/category/children/${parentCategoryId}`);
        ...
        const childProductsPromises = childCategories.map(async (category: any) => {
          const resChildProducts = await fetch(`http://localhost:3000/products/category/${category._id}`);
...
export const getGroupedSaleProducts = async (
  ...
  try {
    // 1. Lấy sản phẩm từ danh mục cha
    const resParentProducts = await fetch(`http://localhost:3000/products/category/${parentCategoryId}`);
    ...
    const childrenData = await Promise.all(
      selectedChildren.map(async (cate) => {
        const res = await fetch(`http://localhost:3000/products/category/${cate._id}`);
    ...
    export const getRelatedProducts = async (productId: string): Promise<IProduct[]> => {
      try {
        const res = await fetch(`http://localhost:3000/products/related/${productId}`);
        if (!res.ok) {
