...
export const getParentCategoryBySlug = async (
  ...
  try {
    const res = await fetch(`https://fiyo.click/api/category/slug/${slug}`);
    if (!res.ok) {
...
export const getAllCategoryChilds = async(parentId:string)=>{
    let res = await fetch(`https://fiyo.click/api/category/children/${parentId}`);
    let data = await res.json();
    let categories = data.map((category:ICategory) =>{
        return{
            _id:category._id,
...
export const getCategoryBySlugs = async (
  ...
  try {
    const res = await fetch(`https://fiyo.click/api/category/${parentSlug}/${childSlug}`);
...
export const getAllProductByParentSlug = async (
  ...
  try {
    const res = await fetch(
      `https://fiyo.click/api/category/parent-with-products/${parentSlug}`,
      { cache: "no-store" }
    );
