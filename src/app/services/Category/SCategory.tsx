...
export const getAllCategoryChilds = async(parentId:string)=>{
    let res = await fetch(`http://localhost:3000/api/category/children/${parentId}`);
    let data = await res.json();
    let categories = data.map((category:ICategory) =>{
        return{
            _id:category._id,
...
export const getCategoryBySlugs = async (
  ...
  try {
    const res = await fetch(`http://localhost:3000/api/category/${parentSlug}/${childSlug}`);
...
export const getAllProductByParentSlug = async (
  ...
  try {
    const res = await fetch(
      `http://localhost:3000/api/category/parent-with-products/${parentSlug}`,
      { cache: "no-store" }
    );
